...
char *post_processor_float(char *float_str, parse_state_t *state)
{
    ...
    int new_size = (my_strlen(new_res) > state->width ?
...
{
    int new_size = my_strlen(str) > state->width ?
        my_strlen(str) : state->width;
    if (state->width == 0 && state->precision > 0 &&
        state->precision < my_strlen(str))
...
char *post_processor_int(char *res, parse_state_t *state)
{
    ...
    int new_size = (my_strlen(res) > state->width ?
        my_strlen(res) : state->width) +
